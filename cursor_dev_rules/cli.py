...
def get_rules_path():
    ...
    # Fallback: look for rules in the project root (for development)
    ...
    try:
        ...
        with Progress(
            ...
            if hasattr(general_rule_source, 'is_file'):
                ...
                # Regular Path object
                general_exists = general_rule_source.exists() and general_rule_source.is_file()
            ...
            if hasattr(specific_rule_source, 'is_file'):
                ...
                # Regular Path object
                specific_exists = specific_rule_source.exists() and specific_rule_source.is_file()
            ...
            if not specific_exists:
                console.print(
                    Panel(
                        ...
                        f"  • Check the rules/{category}/ directory",
                        title="[red]Error[/red]",
                        border_style="red"
                    )
                )
            ...
            task2 = progress.add_task(f"Copying general rule...", total=None)
            general_dest = cursor_rules_dir / "general" / "RULE.md"
            if copy_rule_file(general_rule_source, general_dest, console):
                progress.update(task2, completed=True)
                console.print(f"[green]✓[/green] Copied general rule to [bold]{general_dest.relative_to(cwd)}[/bold]")
            else:
                raise click.Abort()
            ...
            if copy_rule_file(specific_rule_source, specific_dest, console):
                ...
                console.print(f"[green]✓[/green] Copied {framework} rule to [bold]{specific_dest.relative_to(cwd)}[/bold]")
            else:
                raise click.Abort()
